...
This subclasses :class:`MOQTransport` and reaches into a few of its protected
attributes (``_broadcast_path``, ``_peer_broadcast``, ``_publish_broadcast``,
``_call_event_handler``, ...) to replace connection bring-up while reusing its
per-session media engine. Those attributes aren't a stable pipecat-ai API, so
treat this file as a reference implementation to adapt, not a dependency to
pin against — that coupling is also why this lives here as an example rather
than shipping inside pipecat-ai itself.
"""
