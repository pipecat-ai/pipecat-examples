...
### Choosing Between KVS and Third-Party TURN

| Factor | KVS Managed TURN (this example) | Third-party TURN |
|---|---|---|
...
2. For the server:
   ...
   cp env.example .env
   ```
   The server configuration is minimal - the `AGENT_RUNTIME_ARN` will be automatically set during agent deployment. The `KVS_CHANNEL_NAME` should match the agent configuration.
