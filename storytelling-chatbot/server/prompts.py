...
"""Prompts for the storyteller LLM and the image-prompt writer."""

LLM_BASE_PROMPT = """You are a creative storyteller who loves tell whimsical, fantastical stories.
...
# Server messages that tell the client whose turn it is.
CUE_USER_TURN = {"cue": "user_turn"}
CUE_ASSISTANT_TURN = {"cue": "assistant_turn"}

# System instruction for the out-of-band inference that turns a story page
# into an image prompt (see StoryImageProcessor).
IMAGE_PROMPT_INSTRUCTIONS = """You are creating a prompt to generate an image for a child's story book.
...
For any recurring characters, you should provide the same description of them in the image prompt each time, for example: a brown fluffy dog ...
...
FIRST_IMAGE_PROMPT = """Here's the first page of the story:
%s
"""

NEXT_IMAGE_PROMPT = """Here is the text of the story so far:
%s
