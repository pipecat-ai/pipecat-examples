...
Same shape as [`open-telemetry/arize-with-turn-audio`](../arize-with-turn-audio), but the cascade pipeline (Deepgram + OpenAI + Cartesia) is replaced with a single `OpenAIRealtimeLLMService` doing STT + LLM + TTS.
