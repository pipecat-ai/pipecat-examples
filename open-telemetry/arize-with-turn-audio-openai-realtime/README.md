# Arize Tracing with Per-Turn Audio Links — OpenAI Realtime

Same shape as [`open-telemetry/arize-with-turn-audio`](../arize-with-turn-audio), but the cascade pipeline (Deepgram + Google Gemini + Cartesia) is replaced with a single `OpenAIRealtimeLLMService` doing STT + LLM + TTS.
...
- Service accounts: OpenAI (Realtime API), AWS S3
...
| --- | --- | --- |
| `OPENAI_API_KEY` | yes | OpenAI Realtime (STT + LLM + TTS) |
...
- Sister examples: [`open-telemetry/arize`](../arize), [`open-telemetry/arize-with-turn-audio`](../arize-with-turn-audio) (cascade pipeline variant), [`audio-recording-s3-multipart-upload`](../../audio-recording-s3-multipart-upload)
