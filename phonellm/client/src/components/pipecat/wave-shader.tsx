...
export function ReactShaderToy({
  ...
  // Uniform locations resolved once after program link; getUniformLocation
  // is a synchronous string lookup too slow for the per-frame path.
  ...
  const initShaders = (fragmentShader: string, vertexShader: string) => {
    ...
    // preProcessFragment has already flagged every uniform the fragment
    // references, so all locations can be resolved right after link. A
    // null location (uniform optimized out as inactive) is cached too —
    // uniform* calls treat it as a no-op.
  ...
  const setUniforms = (timestamp: number) => {
    ...
    if (uniformsRef.current.iTime?.isNeeded) {
      gl.uniform1f(locations[UNIFORM_TIME] ?? null, (timerRef.current += delta));
    }
      ...
      (entries) => {
        for (const entry of entries) {
          ...
          if (entry.isIntersecting) {
            // Cancel any scheduled frame first: the observer's initial
            // notification also fires while the init loop is already
            // running, and two seeds mean every frame drawn twice.
